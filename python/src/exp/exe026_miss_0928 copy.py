...
# x_train = x_train.drop(["Gender", "Age"], axis=1).copy()
# Genderを数値化
# %%特徴量作成
...
features = x_train9.columns
print("特徴量生成前：", x_train9.shape)
train_ = create_features(x_train9, features, "disease")
...
# # 対数処理
# for i in x_train9.columns:
#     x_train9[i] = np.log10(x_train9[i] + 1)

# # 標準化 pca,UMAPの適用のため 結合データはもとの標準化前のデータにする
# std2 = StandardScaler().fit_transform(x_train9)
# df_std2 = pd.DataFrame(std2, columns=x_train9.columns)
...
# %%
# df_train.columns = ["pc01", "pc02", "pc03", "pc04", "pc05", "pc06", "pc07", "pc08"]
select_list = []
for i in ["pc01", "pc02", "pc03", "pc04", "pc05", "pc06", "pc07", "pc08"]:
    select_list.append(i)
    print(select_list)
    x_trains = x_train[select_list]
    print(x_trains.shape)
