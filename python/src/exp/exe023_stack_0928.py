...
# std_tf = StandardScaler().fit_transform(x_train_tf)
# x_train_tf = pd.DataFrame(std_tf, columns=x_train_tf.columns)
...
# データセット
x_train_las = df_train_las[["pred1", "pred2"]]
...
df_train_las[["true", "pred1", "pred2", "pred_ensemble3"]].head()


# %%
def evaluate_ensemble(input_df, col_pred):
    print(
        f'[auc] model1:{roc_auc_score(input_df["true"], input_df["pred1"]):.4f}, model2:{roc_auc_score(input_df["true"], input_df["pred2"]):.4f},  -> ensemble:{roc_auc_score(input_df["true"], input_df[col_pred]):.4f}'
    )
...
# %%
df_test_las = pd.concat([test_pred_lgb, test_pred_tf], axis=1)
df_test_las.columns = ["pred1", "pred2"]
