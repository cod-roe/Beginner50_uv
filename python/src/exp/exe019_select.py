...
def train_lgb(
    ...
    for nfold in list_nfold:
        ...
        # # 各特徴量のSHAP値の平均絶対値で重要度を算出
        # _shap_importance = np.abs(shap_values).mean(axis=0)
        # # display(_shap_importance)
        # # print(_shap_importance.shape)
        # _shap = pd.DataFrame(
...
df_pc.head()
df_pc.columns = ["pc01", "pc02", "pc03", "pc04"]
...
x_train = x_train[list(shap_sort["col"][:10])]
train_oof, imp, metrics = train_lgb(
    x_train,
    y_train,
    id_train,
...
# umap = umap.UMAP(n_components=4, random_state=123)
# X_embedded = umap.fit_transform(x_test)
...
#%%
x_test = x_test[list(shap_sort["col"][:10])]
x_test.shape
# %%
# drop_lists = ["AST/ALT_ex2", "ALT_GPT", "Alb", "pc04", "pc03", "pc01"]  
...
sns.heatmap(x_train.drop("Gender", axis=1).corr().round(2),vmax=1,vmin=-1,annot=True)
# %%
